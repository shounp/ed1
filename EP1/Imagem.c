...
Imagem *alocaImagem(int largura, int altura){
    int i;
    Imagem *im = mallocSafe(sizeof(Imagem));
    im->largura = largura;
    im->altura = altura;
    im->pixel = mallocSafe(altura*sizeof(Pixel*));
    for(i = 0; i < altura; i++){
        im->pixel[i] = mallocSafe(largura*sizeof(Pixel));
    }
    return im;
}
...
Imagem* copiaImagem(Imagem *origem){
   int i,j;
   if (origem != NULL){
        Imagem *cim = mallocSafe(sizeof(Imagem));
        cim->largura = origem->largura;
        cim->altura = origem->altura;
        cim->pixel = mallocSafe(origem->altura*sizeof(Pixel*));
        for(i = 0; i < origem->altura; i++){
            cim->pixel[i] = mallocSafe(origem->largura*sizeof(Pixel));
        }
        for (i = 0; i < cim->altura; i++) {
            for (j = 0; j < cim->largura; j++) {
                cim->pixel[i][j].cor[RED] = origem->pixel[i][j].cor[RED];
                cim->pixel[i][j].cor[GREEN] = origem->pixel[i][j].cor[GREEN];
                cim->pixel[i][j].cor[BLUE] = origem->pixel[i][j].cor[BLUE];
            }
        }
        return cim;
    }
...
void salvaImagem(Imagem *img, const char *nomeArquivo){
    ...
    fprintf(arquivo, "#EP1 - Estrutura de Dados 1 - Gabriel Martins Sposito\n");
